...
exports.run = (bot, message, args, tools) => { // This is the same handler we have been using in the previous episodes.
  
  message.delete();
    // First, we want to make sure the user put some text as input.
  if (!args[0]) return message.channel.send('Please input some text').then(msg => msg.delete(5000)); // This is from the `Global Functions` episode. - This checks to see if args[0] is defined, if it's not it runs the return statement.
  ...
  message.channel.send('Do you mean: ' + member).then(msg => msg.delete(5000)); // Now, we can post in chat what the autocomplete is.
