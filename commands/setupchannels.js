...
module.exports.run = async (bot, message, args) => {
  ...
  message.guild.createChannel("👑 ● Community", `category`).then((cat) => {
    message.guild.createChannel("welcome", 'text').then((ch) => ch.setParent(cat))
    message.guild.createChannel("events", 'text').then((ch) => ch.setParent(cat))
    message.guild.createChannel("💯 Events", 'voice').then((ch) => ch.setParent(cat))
  })
